...
class ValueSerializator:
    def __init__(self, controlled_objects: tp.Iterable[control_objects.ControlObject]):
        self.__meta = self._get_meta(controlled_objects)

    @property
    def meta(self) -> dict[str, gui.WidgetType]:
        return self.__meta

    def _get_meta(self,controlled_objects: tp.Iterable[control_objects.ControlObject])\
            -> tp.OrderedDict[str, gui.WidgetType]:
        """
        :param controlled_objects: controlled_objects
        :return: None
        """
        meta = OrderedDict()
        for control_object in controlled_objects:
    ...
    def gui_out_repr(self, data: dict[str: float]) -> tp.OrderedDict[str, tp.Any]:
        gui_output = OrderedDict()
    ...
    def gui_names(self) -> list[str]:
        out_names = []
        for i, name in enumerate(self.meta):
            gui_name = f"{i+1}) {name}"
            out_names.append(gui_name)
        return out_names

    def gui_names_type(self) -> dict[str, gui.WidgetType]:
...
class Monitor:
    ...
    def _update_coordinates(self, control_object: control_objects.ControlObject, frame: np.ndarray):
    ...
    def process_view(self, frame: np.ndarray,
                      controlled_objects: tp.Iterable[control_objects.ControlObject]) -> np.ndarray:
        """
    ...
    def process_similarities(self, frame: np.ndarray,
                             controlled_objects: tp.Iterable[control_objects.ControlObject],
    ...
    def _get_values(self, frame: np.ndarray,
        ...
        update_data = {
            controlled_object.name: controlled_object.get_value(frame) for controlled_object in controlled_objects}
        return update_data

    def _procces_data(self, controlled_objects: tp.Iterable[control_objects.ControlObject], update_pos: bool
                      ) -> tuple[np.ndarray, dict[str, float]]:
        frame = self.vid.capture_frame()
        self.process_similarities(frame, controlled_objects, update_pos)
        update_data = self._get_values(frame, controlled_objects)
        return frame, update_data

    def run_loop(self, controlled_objects: tp.Iterable[control_objects.ControlObject],
        ...
        :return:
        """
        serilizator = ValueSerializator(controlled_objects)
        with UpdateManager(serilizator=serilizator,
            ...
            while True:
                frame, update_data = self._procces_data(controlled_objects, update_pos)
